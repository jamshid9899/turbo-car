...
export class PropertyService {
			...
			.aggregate([
				...
				},
				{
					$project: {
						list: 1,
						totalCount: {
							$ifNull: [{ $arrayElemAt: ['$metaCounter.totalCount', 0] }, 0],
						},
					},
				},
			])
	...
	private shapeMatchQuery(match: T, search: PropertySearchFilter): void {
		...
		if (minYear != null || maxYear != null) match.propertyYear = { $gte: minYear ?? 1900, $lte: maxYear ?? 2100 };
		...
		if (isForRent === true) {
			...
			if (rentStartDate) {
				match.$or = [{ rentedUntil: null }, { rentedUntil: { $lte: new Date(rentStartDate) } }];
			}
		}
	...
	public async updatePropertyByOwner(ownerId: ObjectId, input: Partial<PropertyUpdate>): Promise<Property> {
		const result = await this.propertyModel
			.findOneAndUpdate(
				{
					_id: input._id,
